<template>
  <div>
    <PageHead title="情绪日志" />
    <!-- :formItem="formItem"传给子组件，props接收 -->
    <TableSearch :formItem="formItem" @search="handleSearch" />
  <el-table :data="tableData" style="width: 100%">
    <el-table-column prop="id" label="用户ID" width="80" />
    <el-table-column label="会话ID" width="80">
      <!-- 作用域插槽，scope是element表格内部子组件传递的对象， -->
    ...
    <el-table-column label="情绪评分" >
        <template #default="scope">
          <!-- 评分组件，查看element-plus文档 -->
          <el-rate :model-value="scope.row.moodScore" :max="10"></el-rate>
        </template>
    </el-table-column>
    ...
    <el-table-column prop="diaryContent" label="日记内容" />
     <el-table-column label="操作" width="240" fixed="right">
      <template #default="scope">
          <el-button @click="viewSessionDetail(scope.row)" text type="primary" size="large" >详情</el-button>
          <el-button @click="handleDelete(scope.row)" text type="danger" size="large" >删除</el-button>
      </template>
    </el-table-column>
  </el-table>
      <el-pagination 
      ...
      <div class="detail-content"  v-if="currentDetail">
        ...
        <div class="detail-section">
          <h4>时间信息</h4>
          <el-descriptions :column="2" border>
            <el-descriptions-item label="创建时间">{{ currentDetail.createdAt || '-'}} / 5</el-descriptions-item>
            <el-descriptions-item label="更新时间">{{ currentDetail.updatedAt || '-'}} / 5</el-descriptions-item>
          </el-descriptions>
        </div>
      </div>
      <template #footer>
...
const viewSessionDetail = (row) => {
  ...
  if(row.aiEmotionAnalysis) {
    ...
    console.log(aiData.value,"aiData.value")
  }
  else {
    aiData.value = {}
  }
